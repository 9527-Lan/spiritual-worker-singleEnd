<template>
	<view>
		...
		<view style="padding-top: 198rpx;">
			<mp-html ref="article" container-style="padding:20px;min-height:calc(100vh - 130px)" :content="content"
				domain="https://mp-html.oss-cn-hangzhou.aliyuncs.com" :tag-style="tagStyle" :editable="editable"
				@remove="remove" />
		</view>
		<block v-if="modal">
			<view class="mask" />
...
<script>
	...
	export default {
		...
		onLoad(event) {
			event.data?this.content = event.data:null
		},
		methods: {
			// 删除图片/视频/音频标签事件
			remove(e) {
			...
			save() {
				setTimeout(() => {
					if (this.editable) {
						...
						// 	}
						// })
						uni.setStorageSync('workPlace',content);
